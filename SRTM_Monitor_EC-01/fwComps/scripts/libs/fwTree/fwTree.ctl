...
{
  string node_name, parent_name, old_parent_name, tree;
  dyn_string children;
  int index;
  ...
  // The following statement triggers a treeRedo to other managers, so that the information is passed on even if this action
  // is executed from a script.
////  dpSetWait("ToDo.treeRedos", parent);

//  dyn_string exceptionInfo2;
//  fwTree_getTreeName(parent, tree, exceptionInfo2);
//  if(!dynlen(exceptionInfo2)) {
//    dpSetWait("fwTT_"+tree+".treeRedos", parent);
//  } else {
//    dynAppend(exInfo,exceptionInfo2);
//  }
}
...
_fwTree_removeNode(string parent, string node, dyn_string &exInfo)
{
  ...
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_removeNode(): node "+node+" does not exist");
  }
...
//	clipboard_name = _fwTree_makeNodeName(clipboard);
  ...
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_cutNode(): node "+node+" does not exist");
  }
  if(dpExists(parent_name))
  {
    _fwTree_getChildren(parent_name, children);
...
_fwTree_renameNode(string node, string new_node, dyn_string &exInfo)
{
  ...
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_renameNode(): node "+node+" does not exist");
  }
}
...
fwTree_getChildren(string parent, dyn_string &nodes, dyn_string &exInfo)
{
  ...
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_getChildren(): node "+parent+" does not exist");
  }
}
...
fwTree_getParent(string node, string &parent, dyn_string &exInfo)
{
  ...
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_getParent(): node "+node+" does not exist");
  }
}
...
//	node_name = _fwTree_makeNodeName(node);
  ...
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_getCUName(): node "+node+" does not exist");
  }
}
...
fwTree_setNodeDevice(string node, string device, string type, dyn_string &exInfo)
{
  ...
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_setNodeDevice(): node "+node+" does not exist");
  }
}
...
fwTree_getNodeCUDevice(string node, int &cu, string &device, string &type, dyn_string &exInfo)
{
  ...
  if(dpExists(node_name))
  {
    dpGet(node_name+".cu",cu, node_name+".device",dev_dp, node_name+".type",type);
    device = dev_dp;
  }
  else
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_getNodeDevice(): node "+node+" does not exist");
  }
}
...
fwTree_getNodeDevice(string node, string &device, string &type, dyn_string &exInfo)
{
  ...
  if(dpExists(node_name))
  {
    dpGet(node_name+".device",dev_dp,node_name+".type",type);
    device = dev_dp;
  }
  else
  {
...
fwTree_setNodeUserData(string node, dyn_string data, dyn_string &exInfo)
{
  ...
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_setNodeUserData(): node "+node+" does not exist");
  }
}
...
fwTree_getNodeUserData(string node, dyn_string &data, dyn_string &exInfo)
{
  ...
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_getNodeUserData(): node "+node+" does not exist");
  }
}
...
fwTree_setNodeCU(string node, int cu, dyn_string &exInfo)
{
  ...
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_setNodeCU(): node "+node+" does not exist");
  }
}
...
fwTree_getNodeCU(string node, int &cu, dyn_string &exInfo)
{
  ...
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_getNodeCU(): node "+node+" does not exist");
  }
}
...
string fwTree_getNodeDisplayName(string node, dyn_string exInfo)
{
  ...
  if(!dpExists(node_name))
  {
    _fwTree_exception(exInfo,"WARNING","fwTree_getNodeDisplayName(): node "+node+" does not exist");
  }
  if(strpos(node,"&") == 0)
    obj = substr(node,5);
  return(obj);
