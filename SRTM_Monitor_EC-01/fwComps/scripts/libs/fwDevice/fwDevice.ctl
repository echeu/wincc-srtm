...
#uses "classes/fwDevice/FwDeviceCache"
...
global mapping fwDevice_deviceDpType;
...
@param exceptionInfo	details of any exceptions
*/
void fwDevice_getDpType(const string deviceType, string &deviceDpType, dyn_string &exceptionInfo)
{
  deviceDpType = fwDevice_deviceDpType.value(deviceType, "");
  if (!deviceDpType.isEmpty())
  {
    return;
  }

  string query = "SELECT 'dpType:_online.._value' FROM '*' WHERE _DPT = \"_FwDeviceDefinition\" AND 'type:_online.._value' = \"" + deviceType + "\"";
  dyn_dyn_anytype data;
  dpQuery(query, data);

  int dataLenght = dynlen(data);
  if (dataLenght == 2)
  {
    deviceDpType = data[2][2];
    fwDevice_deviceDpType[deviceType] = deviceDpType;
  }
  else if (dataLenght > 2)
  {
    fwException_raise(exceptionInfo,
                      "ERROR",
                      __FUNCTION__ + ": multiple entries found for device type '" + deviceType + "'",
                      "");
    return;
  }
}
...
@param exceptionInfo		details of any exceptions
*/
void fwDevice_getHierarchy(const string &deviceDpName, dyn_dyn_string &deviceHierarchy, dyn_string &exceptionInfo) {
  bool cacheHit;
  deviceHierarchy = FwDeviceCache::getValueOrDefault(deviceDpName, "hierarchy", "", cacheHit);
  if (cacheHit)
  {
    return;
  }

  deviceHierarchy.clear();
  if (deviceDpName.isEmpty()) {
    fwException_raise(exceptionInfo, "ERROR", "Device DP name is blank", "");
    deviceHierarchy = makeDynString("");
    return;
  }

  const string parentPattern = "([^/]*)/?";
  const string numberPattern = "(\\d+)(?!.*[:\\d])";
  string parent = "";

  int devceDpNameLength = deviceDpName.length();
  int deviceDpNamePosition = 0;
  while (deviceDpNamePosition < devceDpNameLength) {
    dyn_string result;
    mapping options = makeMapping("startPosition", deviceDpNamePosition);
    int rc = regexpSplit(parentPattern, deviceDpName, result, options);
    if (rc < 0) {
      result = makeDynString(deviceDpName, deviceDpName);
    }

    deviceDpNamePosition += result[1].length();
    parent += (parent.isEmpty() ? "" : "/") + result[2];

    dyn_string number;
    rc = regexpSplit(numberPattern, result[2], number);
    if (rc < 0) {
      deviceHierarchy.insertAt(0, makeDynString(parent, "", ""));
    } else {
      deviceHierarchy.insertAt(0, makeDynString(parent, number[2], number[2].isEmpty() ? "" : (int)number[2]));
    }
  }

  FwDeviceCache::setEntry(deviceDpName, "hierarchy", deviceHierarchy);
}
...
*/
void fwDevice_getModel(const dyn_string &device, string &model, dyn_string &exceptionInfo)
{
  ...
  bool cacheHit;
  model = FwDeviceCache::getValueOrDefault(device[fwDevice_DP_NAME], "model", "", cacheHit);
  if (!cacheHit)
  {
    ...
    if (dpExists(dpe))
    {
      ...
      if (!model.isEmpty())
      {
        FwDeviceCache::setEntry(device[fwDevice_DP_NAME], "model", model);
      }
    }
  }
}
...
@param exceptionInfo	details of any exceptions
*/
void fwDevice_getModelProperties(dyn_string device, dyn_dyn_string &modelProperties, dyn_string &exceptionInfo)
{
//  time t1, t2, t3, t4;
...
//  t1 = getCurrentTime();
  ...
  bool hasDeviceDpName = device.count() >= fwDevice_DP_NAME && !device[fwDevice_DP_NAME].isEmpty();
  if (hasDeviceDpName)
  {
    bool cacheHit;
    dyn_dyn_string emptyDynDynString;
    modelProperties = FwDeviceCache::getValueOrDefault(device[fwDevice_DP_NAME], "modelProperties", emptyDynDynString, cacheHit);
    if (cacheHit)
    {
      return;
    }
  ...
  bool knownModel = dynlen(modelDp) == 3;
...
//   DebugN("fwDevice_getModelProperties(): ", t4 - t3, t3 - t2, t2 - t1);
  ...
  if (knownModel && hasDeviceDpName)
  {
    FwDeviceCache::setEntry(device[fwDevice_DP_NAME], "modelProperties", modelProperties);
  }
}
...
@param exceptionInfo	details of any exceptions
*/
fwDevice_getName(const string &deviceDpName, string &deviceName, dyn_string &exceptionInfo)
{
  if (deviceDpName.isEmpty())
  {
    deviceName = "";
  ...
  int delimiterPosition = deviceDpName.lastIndexOf(fwDevice_HIERARCHY_SEPARATOR);
  if (delimiterPosition > -1)
  {
    deviceName = deviceDpName.mid(delimiterPosition + 1);
  }
  else
  {
    int colonPosition = deviceDpName.indexOf(":");
    deviceName = deviceDpName.mid(colonPosition + 1);
  }
}
...
@param exceptionInfo	details of any exceptions
*/
void fwDevice_setModel(const string &deviceDpName, const string &deviceModel, dyn_string &exceptionInfo)
{
  string deviceModelDpe = deviceDpName + ".model";
  if (dpExists(deviceModelDpe))
  {
    int rc = dpSet(deviceModelDpe, deviceModel);
    if (rc == 0 && dynlen(getLastError()) == 0)
    {
      FwDeviceCache::setEntry(deviceDpName, "model", deviceModel);
    }
  }
  else
  {
