...
// Global Variables
// ----------------
const string g_sProgressDpName = "fwProgressBar";
...
// ---------------
	...
	ChildPanelOnCentralModal("fwGeneral/fwProgressBar.pnl", sTitle, makeDynString("$sDpName:" + sSystemName + g_sProgressDpName));

	// Return to calling routine
	return;
}

void fwShowProgressBar(string sMessage = "", float fPercentage = 0.0)
{
// Local Variables
// ---------------
	string sSystemName = getSystemName();
...
void fwProgressBar_ShowEmbeddedProgressBar(int iXPos, int iYPos)
{
	if (!shapeExists("progressBarReference")) {
		addSymbol(myModuleName(), myPanelName(),
				  "fwGeneral/fwGenericEmbeddedProgressBar.pnl",
				  "progressBarReference",
				  makeDynString("$iBarWidth:200"),
				  iXPos, iYPos, 1, 1, 1);
	}
}
...
void fwProgressBar_HideEmbeddedProgressBar()
{
	if (shapeExists("progressBarReference")) {
		removeSymbol(myModuleName(), myPanelName(),
					 "progressBarReference");
	}
}
