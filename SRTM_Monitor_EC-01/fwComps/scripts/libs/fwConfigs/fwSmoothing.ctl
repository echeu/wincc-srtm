					...
					dyn_string &exceptionInfo, bool runDriverCheck = FALSE, bool storeInParamHistory = TRUE)
{
	if (isDollarDefined("$bIgnoreDriverRelatedConfigs"))      //this check is done in order to make UNICOS CPC skip the set
	{
		...
		{
			fwException_raise(exceptionInfo, "INFO", "fwSmoothing_setMany(): the following dpes won't be configured (due to parameter bIgnoreDriverRelatedConfigs)", "");
			DebugTN("fwSmoothing_setMany(): list of dpes not configured:", dpes);
			return ;
		}
	}
	...
	if(runDriverCheck)
	{
		...
		if(!areAccessible)
		{
			...
			fwException_raise(exceptionInfo, "ERROR", "fwSmoothing_setMany: To set the smoothing config, driver number " + errorString + " must be running.", errorString);
			return;
		}
	}
...
fwSmoothing_deleteMany(dyn_string dpes, dyn_string &exceptionInfo)
{
	...
	{
		_fwConfigs_convertDriverNumbersToErrorMessage(requiredDrivers, errorString, exceptionInfo);
		fwException_raise(exceptionInfo, "ERROR", "fwSmoothing_deleteMany: To delete the smoothing config, driver number " + errorString + " must be running.", errorString);
		return;
	}
...
fwSmoothing_get(string dpe, bool &configExists, int &smoothProcedure, float &deadband, float &timeInterval, dyn_string &exceptionInfo)
{
	...
	{
		_fwConfigs_convertDriverNumbersToErrorMessage(requiredDrivers, errorString, exceptionInfo);
		fwException_raise(exceptionInfo, "ERROR", "fwSmoothing_get: To access the smoothing config, driver number " + errorString + " must be running.", errorString);
		return;
	}
...
@param exceptionInfo	details of any errors are returned here
@param procedureIdx      (optional, default 1, only meaningful if @ref isArchiveConfig is set) - archiving procedure index (detail in the archive config),
                         should always be set to 1 unless NGA archiving is used.
*/
_fwSmoothing_getParameters(dyn_string &dpes, bool isArchiveConfig, dyn_int &smoothProcedure, dyn_float &deadband, dyn_float &timeInterval, dyn_string &exceptionInfo, int procedureIdx = 1)
{
	string configString;
	dyn_string dsDpAttr;
	...
	if(isArchiveConfig)
	{
		configString = ":_archive." + (string)procedureIdx + ".";
	}
	else
	{
		configString = ":_smooth..";
...
//       if(i>1150)DebugN(dpes[i]);
			...
			switch(iSmoothType)
			{
				...
				case DPATTR_OLD_NEW_AND_TIME_SMOOTH:
				case DPATTR_OLD_NEW_OR_TIME_SMOOTH:
					smoothProcedure[i] = daAttrVal[k];
					k++;
					deadband[i] = daAttrVal[k];
					k++;
					timeInterval[i] = daAttrVal[k];
					k++;
					break;
				default:
					smoothProcedure[i] = 0;
...
@param exceptionInfo	details of any errors are returned here
@param storeInParamHistory		Optional parameter (default value = TRUE) - TRUE to archive the parameter change, else FALSE
@param procedureIdxs      (optional, default: empty, only meaningful if @ref isArchiveConfig is set) - archiving procedure indexes (details in the archive config),
                          if empty, index 1 will be used for all DPEs.
*/
_fwSmoothing_setParameters(dyn_string dpes, bool isArchiveConfig, dyn_int smoothProcedure,
						   ...
						   bool storeInParamHistory = TRUE, dyn_int procedureIdxs = makeDynInt())
{
	int i, length;
	dyn_errClass errors;
	string configString;
	...
	for(i = 1; i <= length; i++)
	{
      	if(isArchiveConfig)
      	{
      		configString = ":_archive." + (i <= dynlen(procedureIdxs) ? procedureIdxs[i] : "1") + ".";
      	}
      	else
      	{
      		configString = ":_smooth..";
