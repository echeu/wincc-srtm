...
@par Modification History
  ...
  - @sav{85465}: Improved performance for fwConfigConversion_getMany()
	...
	switch(configType)
	{
		...
		default:
			fwException_raise(exceptionInfo,
							  "ERROR",
							  "_fwConfigConversion_prepareSet(): Config type is not valid for dpe '" + dpe + "'",
							  "");
			return;
	}
...
// base curve conversion
		...
		case DPDETAIL_CONV_INVERT:
			...
			break;
		default:
			fwException_raise(exceptionInfo, "ERROR", "fwConfigConversion_set(): Invalid conversion type for dpe '" + dpe + "'", "");
			return;
			break;
	}
}
						   ...
						   dyn_dyn_float &arguments, dyn_string &exceptionInfo)
{
	...
	string config, errorString;
	dyn_int requiredDrivers;
	dyn_string dsDpAttr, dsTypesAttr, dsTypesVal, dsStdTypesAttr, dsStdTypesVal;
	dyn_mixed dsAttrVal, dmTempVal;
	int i, j, k, n, length;
	int ret;
	...
	switch(configType)
	{
		...
		default:
			fwException_raise(exceptionInfo,
							  "ERROR",
							  "fwConfigConversion_get(): Config type " + configType + " is not valid",
							  "");
			return;
	}
	...
	{
//DebugN("Grouping...", numberOfSettings, tempAttributes, tempValues);
		dynAppend(dsTypesAttr , dpes[i] + config + ".._type");
		if((dynlen(dsTypesAttr) > fwConfigs_OPTIMUM_DP_SET_SIZE) || (i == length))
		{
	...
	//get the conversion types of all the configured dpes:
	int iValLen = dynlen(dsTypesVal);
	for(i = 1 ; i <= iValLen; i++)
	{
		switch(dsTypesVal[i])
		{
			case DPCONFIG_CONVERSION_RAW_TO_ENG_MAIN:  // message conversion
			case DPCONFIG_CONVERSION_ING_TO_RAW_MAIN:  // command conversion
				dynAppend(dsStdTypesAttr, dpes[i] + config + ".1._type");
				break;
			...
			default:  // in principle, execution should never arrive here
				fwException_raise(exceptionInfo,
								  "ERROR",
								  "fwConfigConversion_get(): Conversion config type " + dsTypesVal[i] + " on dpe '" + dpes[i] + "' not supported",
								  "");
				break;
		}
		if((dynlen(dsStdTypesAttr) > fwConfigs_OPTIMUM_DP_SET_SIZE) || (i == iValLen && dynlen(dsStdTypesAttr) > 0))
		{
			dpGet(dsStdTypesAttr, dsTempVal);
			dynAppend(dsStdTypesVal, dsTempVal);
			dynClear(dsStdTypesAttr);
		}
	}
	...
	for(i = 1 ; i <= length ; i++)
	{
			...
			case DPCONFIG_CONVERSION_ING_TO_RAW_MAIN:  // command conversion
				switch(dsStdTypesVal[j])
				{
					...
					case DPDETAIL_CONV_POLY:
						...
						dynAppend(dsDpAttr, dpes[i] +  config + ".1._poly_grade");
						dynAppend(dsDpAttr, dpes[i] +  config + ".1._poly_a");
						dynAppend(dsDpAttr, dpes[i] +  config + ".1._poly_b");
						dynAppend(dsDpAttr, dpes[i] +  config + ".1._poly_c");
						dynAppend(dsDpAttr, dpes[i] +  config + ".1._poly_d");
						dynAppend(dsDpAttr, dpes[i] +  config + ".1._poly_e");
						break;

					// base curve conversion
					case DPDETAIL_CONV_LIN_INT:
						// configuration parameters
						dynAppend(dsDpAttr, dpes[i] + config + ".1._linint_num");
						dynAppend(dsDpAttr, dpes[i] + config + ".1._linint1_x");
						dynAppend(dsDpAttr, dpes[i] + config + ".1._linint1_y");
						dynAppend(dsDpAttr, dpes[i] + config + ".1._linint2_x");
						dynAppend(dsDpAttr, dpes[i] + config + ".1._linint2_y");
						dynAppend(dsDpAttr, dpes[i] + config + ".1._linint3_x");
						dynAppend(dsDpAttr, dpes[i] + config + ".1._linint3_y");
						dynAppend(dsDpAttr, dpes[i] + config + ".1._linint4_x");
						dynAppend(dsDpAttr, dpes[i] + config + ".1._linint4_y");
						dynAppend(dsDpAttr, dpes[i] + config + ".1._linint5_x");
						dynAppend(dsDpAttr, dpes[i] + config + ".1._linint5_y");
						break;
					case DPDETAIL_CONV_INVERT:
						break;
					case DPDETAIL_CONV_NONE:
					...
					default:  // not supported conversion types will appear as not exisiting, is this ok?
						fwException_raise(exceptionInfo,
										  "ERROR",
										  "fwConfigConversion_get(): Conversion type " + conversionType + " on dpe '" + dpes[i] + "' not supported",
										  "");
						break;
				}
				j++;
		...
		{
			ret += dpGet(dsDpAttr, dmTempVal);
			dynAppend(dsAttrVal, dmTempVal);
			dynClear(dsDpAttr);
		}
	}

//   DebugN("fwConfigConversion_getMany - available params:\n",dsDpAttr, dsAttrVal);

	if(ret < 0)
	{
		fwException_raise(exceptionInfo,
						  "ERROR",
						  "fwConfigConversion_get(): Could not perform dpGet. Check lock status of the dpes.",
						  "");
		return;
	}
		...
		switch(dsTypesVal[i])
		{
			...
			case DPCONFIG_CONVERSION_ING_TO_RAW_MAIN:  // command conversion
				switch(dsStdTypesVal[j])
				{
					case DPDETAIL_CONV_POLY:
						configExists[i] = true;
						conversionType[i] = dsStdTypesVal[j];
						order[i] = dsAttrVal[k];
						k++;
						for(n = 1 ; n <= 5 ; n++) //5 parameters
						{
							arguments[i][n] = dsAttrVal[k];
							k++;
						}
						break;
					case DPDETAIL_CONV_LIN_INT:
					...
					default:
						fwException_raise(exceptionInfo, "ERROR",
										  "fwConfigConversion_get(): Conversion type " + dsStdTypesVal[j] + " on dpe '" + dpes[i] + "' not suppported", "");
						break;
				}
				j++;
				break;
...
//     DebugN("fwConfigConversion_getMany - sorted param for: "+dpes[i],
//            "configExists[i]: "+configExists[i],
//            "conversionType[i]: "+conversionType[i],
//            "order[i]: "+order[i],
...
// fwConfigConversion_getManyOld(dyn_string &dpes, dyn_bool &configExists, int configType, dyn_int &conversionType, dyn_int &order,
// 					   								dyn_dyn_float &arguments, dyn_string &exceptionInfo)
// {
// 	int i, length;
//
