...
//int iMode; bool &cmwLowlevel, int iDir, int iMode,
	...
	// FWCORE-3486; driver number and mode/direction in the addressConfig needs to be int, while the addrconfig element is char...
	...
	addressConfig[fwPeriphAddress_CMW_MODE] = drvMode;
...
fwPeriphAddressCMW_check(dyn_anytype dsParameters, dyn_string& exceptionInfo)
{
 ...
 //DebugTN(__FUNCTION__, dsParameters);
	...
	if (dynlen(dsParameters) == FW_PARAMETER_FIELD_NUMBER + 1)
	{
		...
		// 2. Driver number
		if (sscanf((int)dsParameters[FW_PARAMETER_FIELD_DRIVER], "%d", driverNum) <= 0)
		{
		...
		// 4. Address subindex
		const int MAX_ALLOWED_CMW_SUBINDEX = 65533;
		if (sscanf((int)dsParameters[FW_PARAMETER_FIELD_SUBINDEX], "%d", addressSubindex) <= 0)
		{
			fwException_raise(exceptionInfo, "ERROR", "fwPeriphAddressCMW_check: " + getCatStr("fwPeriphAddressCMW", "CMWBADSUBINDEX"), "");
		}
		else if (addressSubindex > MAX_ALLOWED_CMW_SUBINDEX)
		{
			fwException_raise(exceptionInfo, "ERROR", "fwPeriphAddressCMW_check: " + getCatStr("fwPeriphAddressCMW", "CMWSUBINDEXTOOLARGE"), "");
		}
		// 5. Mode
		if (sscanf((int)dsParameters[fwPeriphAddress_CMW_MODE], "%d", mode) <= 0)
		{
		...
		// 7. Interval Time
		if (sscanf((int)dsParameters[FW_PARAMETER_FIELD_INTERVAL], "%d", intervalTime) <= 0)
		{
			fwException_raise(exceptionInfo, "ERROR", "fwPeriphAddressCMW_check: " + getCatStr("fwPeriphAddressCMW", "CMWBADINTERVAL"), "");
		}
		// 8. Data type
		if (sscanf((int)dsParameters[FW_PARAMETER_FIELD_DATATYPE], "%d", dataType) <= 0)
		{
			fwException_raise(exceptionInfo, "ERROR", "fwPeriphAddressCMW_check: " + getCatStr("fwPeriphAddressCMW", "CMWBADDATATYPE"), "");
		}
		// 9. Address active
