...
fwOriginal_setMany(dyn_string &dpes, dyn_anytype &values, dyn_dyn_bool &infoBits, dyn_string &exceptionInfo, string mode = "VALUE_VARIABLE_BITS_USER_BITS")
{
	...
	length = dynlen(dpes);
	_fwConfigs_getConfigTypeAttribute(dpes, fwConfigs_PVSS_ORIGINAL, activeBits, localExceptionInfo, ".._active");
    if (dynlen(localExceptionInfo)) {
		fwException_raise(exceptionInfo, "ERROR", "fwOriginal_setMany(): Could not get some of the attributes. Function aborted.", "");
