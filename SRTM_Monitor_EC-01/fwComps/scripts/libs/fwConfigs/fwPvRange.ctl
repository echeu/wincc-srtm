...
@par Modification History
  ...
  - @sav{86287}: fwPVRange_objectGet() returns incomplete data. The parameters object is now initialized in _fwPvRange_get().
				...
				dyn_dyn_anytype &configData, dyn_string &exceptionInfo)
{
	...
	dyn_anytype rawData;
	dyn_string attributesToRead, tempAttributesToRead, localExceptionInfo;
	dyn_int dpeInfo;
	...
	{
		fwException_raise(localExceptionInfo, "ERROR", "_fwPvRange_get(): Could not get some of the attributes. Function aborted.", "");
		return;
	}
	for(i = 1; i <= length; i++)
	{
		...
		configData[i][ fwPvRange_MINIMUM_VALUE	] = 0.0;
		configData[i][ fwPvRange_MAXIMUM_VALUE	] = 0.0;
		configData[i][ fwPvRange_VALUE_SET 			] = makeDynFloat();
		configData[i][ fwPvRange_VALUE_PATTERN 	] = "";
		configData[i][ fwPvRange_NEGATE_RANGE 	] = false;
		configData[i][ fwPvRange_IGNORE_OUTSIDE	] = false;
		configData[i][ fwPvRange_INCLUSIVE_MINIMUM	] = false;
		configData[i][ fwPvRange_INCLUSIVE_MAXIMUM	] = false;
...
@param minValue				list of lower limit for the range. Passed as reference only for performance reasons. Not modified.
@param maxValue				list of upper limit for the range. Passed as reference only for performance reasons. Not modified.
@param negateRange		list of bools, TRUE for "reverse (negate) value-range handling" . Passed as reference only for performance reasons. Not modified.
@param ignoreOutside	list of bools, TRUE when "invalid values should be ignored". Passed as reference only for performance reasons. Not modified.
@param inclusiveMin		list of bools, TRUE when "minimum value included in the value range". Passed as reference only for performance reasons. Not modified.
@param inclusiveMax		list of bools, TRUE when "maximum value included in the value range". Passed as reference only for performance reasons. Not modified.
@param exceptionInfo	details of any errors are returned here
*/
					...
					dyn_bool &ignoreOutside, dyn_bool &inclusiveMin, dyn_bool &inclusiveMax, dyn_string &exceptionInfo)
{
	...
	fwPvRange_getObjectMany(dpes, configExists, dynPvRangeType, dynConfigData, localExceptionInfo);
	if(dynlen(localExceptionInfo))
	{
		fwException_raise(exceptionInfo, "ERROR", "fwPvRange_getMany(): Could not get some of the attributes. Function aborted.", "");
