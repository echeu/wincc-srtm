...
@par Modification History
  ...
  - @jira{FWCORE-3110}: redundancy

  24/05/2011 Marco Boccioli
  - @jira{ENS-3185}: support alarms for dpe of type bit32.
...
#uses "fwConfigs/fwConfigConversion.ctl"
#uses "fwConfigs/fwDpFunction.ctl"
#uses "fwConfigs/fwFormat.ctl"
#uses "fwConfigs/fwOriginal.ctl"
#uses "fwConfigs/fwPeriphAddress.ctl"
#uses "fwConfigs/fwPvRange.ctl"
#uses "fwConfigs/fwSmoothing.ctl"
#uses "fwConfigs/fwUnit.ctl"
...
/** Optimized dpGet for many DPEs

    This function performs a dpGet for a masive amount of DPEs, making sure that the optimum
    splitting of dpGet is done, and combining the results

@param dpes 	List of datapoint elements to be queried
@param values	The values will be stored into this variable
@param exceptionInfo	Standard exception-handling variable
@config 	determines which config type/detail to use; for instance "_archive.._archive"
		if empty, then the values will be returned; one could also specify the
		leading ":" in the config name (":_archive.._archive");
*/
fwConfigs_dpGetMany(dyn_string dpes, dyn_mixed &values, dyn_string &exceptionInfo, string configName="")
{

    // declare variables for partial results
    dyn_string dpesToGet;

    // config that we specify may or may not contain leading colon
    // this way we could treat the case of getting values (empty configName)
    // in the loops below
    if (configName!="") {
	if (substr(configName,0,1)!=":") configName=":"+configName;
    }


    int length=dynlen(dpes);
    for (int i=1;i<=length;i++) {
	dynAppend(dpesToGet,dpes[i]+configName);
	if((dynlen(dpesToGet) > fwConfigs_OPTIMUM_DP_GET_SIZE) || (i == length)) {
	    dyn_mixed dpesValues;
	    dpGet(dpesToGet, dpesValues);
	    dyn_errClass err=getLastError();
	    if (dynlen(err)) {
		fwException_raise(exceptionInfo,"ERROR","Error encountered while extracting DPEs values/configs - see log for details","");
		throwError(err);
		return;
	    }
	    dynAppend(values,dpesValues);
	    if (i != length) dynClear(dpesToGet);
        }
    }
}
...
@param exceptionInfo		details of any errors are returned here
*/
void _fwConfigs_getConfigAttributes(const dyn_string &dpes, const string &configType, const dyn_string &attributes, dyn_dyn_mixed &attributeValues, dyn_string &exceptionInfo)
{
  dyn_dyn_string configAttributes;

  int numberOfDpes = dynlen(dpes);
  if (numberOfDpes == 0)
  {
  ...
  int numberOfAttributes = dynlen(attributes);
  // resize to the final size directly instead of incremental in a loop
  configAttributes[numberOfDpes] = makeDynString();

  for(int i = 1; i <= numberOfDpes; i++)
  {
    // resize to the final size directly instead of incremental in a loop
    configAttributes[i][numberOfAttributes] = "";
    string dpeWithConfigType = dpes[i] + ":" + configType;
    for(int j = 1; j <= numberOfAttributes; j++)
    {
      configAttributes[i][j] = dpeWithConfigType + attributes[j];
    }
  }
...
_fwConfigs_getAttributes(dyn_dyn_string dpeAttributes, dyn_dyn_mixed &attributeValues, dyn_string &exceptionInfo)
{
    ...
    dyn_mixed valuesRead;
    dyn_string attributesToRead;

    dynClear(attributeValues);

    int numberOfDpes = dynlen(dpeAttributes);

    for (int i=1; i<=numberOfDpes; i++) {
        string dpe=dpSubStr(dpeAttributes[i][1], DPSUB_SYS_DP_EL);
        if (dpe=="" || !dpExists(dpe)) {
            fwException_raise(exceptionInfo, "ERROR", "Cannot retrieve config attribute: "+dpeAttributes[i][1]+
                              " - DPE does not exist", "");
            return;
        }
        numberOfAttributes[i] = dynlen(dpeAttributes[i]);
        int attributeCount;
        for (int j=1; j<=numberOfAttributes[i]; j++) {
        ...
        if ((attributeCount > fwConfigs_OPTIMUM_DP_GET_SIZE) || ((i == numberOfDpes) && (attributeCount != 0))) {
            dyn_mixed tempValuesRead;
            int ret = dpGet(attributesToRead, tempValuesRead);
            dyn_errClass err = getLastError();
            if (ret < 0 || dynlen(err)) {
                fwException_raise(exceptionInfo, "ERROR", "_fwConfigs_getAttributes(): Could not get some of the attributes. "+
                                  getErrorText(err), "");
                return;
            }
...
_fwConfigs_setAttributes(dyn_dyn_string dpeAttributes, dyn_dyn_anytype attributeValues, dyn_string &exceptionInfo, float setWithTime = -1)
{
	...
	for(i = 1; i <= numberOfDpes; i++)
	{
        string dpe=dpSubStr(dpeAttributes[i][1], DPSUB_SYS_DP_EL);
        if (dpe=="" || !dpExists(dpe)) {
            fwException_raise(exceptionInfo, "ERROR", "Cannot set config attribute: "+dpeAttributes[i][1]+
                              " - DPE does not exist", "");
            return;
        }
...
//DebugN(setWithTime, setWithTime < 0);
			...
			{
				throwError(errors);
				fwException_raise(exceptionInfo, "ERROR", "_fwConfigs_setAttributes(): Could not set some attributes: "+
                                  getErrorText(errors), "");
			}
...
fwConfigs_getDataTypeText(int dataType, string &dataTypeText, dyn_string &exceptionInfo)
{
	switch(dataType)
	{
		...
		case DPEL_BIT64:
			dataTypeText = "BIT64";
			break;
		case DPEL_BIT64_STRUCT:
			dataTypeText = "BIT64 Structure";
			break;
		case DPEL_DYN_BIT64:
			dataTypeText = "Dyn BIT64";
			break;
		case DPEL_DYN_BIT64_STRUCT:
			dataTypeText = "Dyn BIT64 Structure";
			break;
		//BLOB Types
		case DPEL_BLOB:
			dataTypeText = "BLOB";
		...
		case DPEL_ULONG:
			dataTypeText = "ULONG";
			break;
		case DPEL_ULONG_STRUCT:
			dataTypeText = "ULONG Structure";
			break;
		case DPEL_DYN_ULONG:
			dataTypeText = "Dyn ULONG";
			break;
		case DPEL_DYN_ULONG_STRUCT:
			dataTypeText = "Dyn ULONG Structure";
			break;
		//INT Types
		case DPEL_INT:
		...
		case DPEL_LONG:
			dataTypeText = "LONG";
			break;
		case DPEL_LONG_STRUCT:
			dataTypeText = "LONG Structure";
			break;
		case DPEL_DYN_LONG:
			dataTypeText = "Dyn LONG";
			break;
		case DPEL_DYN_LONG_STRUCT:
			dataTypeText = "Dyn LONG Structure";
			break;
		//FLOAT Types
		case DPEL_FLOAT:
			dataTypeText = "FLOAT";
 ...
 * @sa fwDPELock_getLocked
 *
 * @param dpe		the data point element in question
 * @param config	the config to check.  Use the following constants to pick:
 *			fwConfigs_PVSS_ADDRESS, fwConfigs_PVSS_ARCHIVE, fwConfigs_PVSS_ALERT_HDL
 *			fwConfigs_PVSS_ORIGINAL, fwConfigs_PVSS_PV_RANGE, fwConfigs_PVSS_SMOOTH
 *			fwConfigs_PVSS_MSG_CONV, fwConfigs_PVSS_CMD_CONV, fwConfigs_PVSS_COMMON
 * @param isLocked	returns TRUE if the config is locked, else FALSE
 * @param lockDetails	if the config is locked, the details of the lock are returned here; see @ref fwDPELock_getLocked
 *                      for more information
 * @param exceptionInfo	details of any errors are returned here
**/
void fwConfigs_getConfigLock(string dpe, string config, bool &isLocked, dyn_string &lockDetails, dyn_string &exceptionInfo)
{
    string dpe_and_config=dpe+":"+config;
    isLocked=fwDPELock_getLocked(dpe_and_config,lockDetails,exceptionInfo);
}
...
bool fwConfigs_init()
{
	if (_fwConfigs_isInitialized) return true;
	...
	_fwConfigs_isInitialized=true;
	return _fwConfigs_isInitialized;
}
