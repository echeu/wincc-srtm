...
#uses "fwConfigs/fwAlertConfigDeprecated.ctl" //import deprecated functions
#uses "fwConfigs/fwConfigConstants.ctl"

// no need anymore for aes.ctl -> FWCORE-3431, fwAlertConfig_isAlertFilteringActive() implemented in this lib
...
@par Modification History
  ...
  - @jira{FWCORE-3219} fwAlertConfig - Problem when setting multiple discrete analog alarm each with a different number of ranges


  04/08/2014 Marco Boccioli
  - @jira{FWCORE-3126} Hysteresis alarms: _fwAlertConfig_generateExtraAnalogConfig(), fwAlertConfig_objectGetMany(), fwAlertConfig_objectSetMany()
  modified to support hysteresis thresholds for analog alarms.

  07/08/2012 Marco Boccioli
  - @jira{FWCORE-3178} _fwAlertConfig_objectCreateParams(), fwAlertConfig_objectCreate*() for text parameters, use langString instead of string

  07/08/2012 Marco Boccioli
  - @jira{FWCORE-3151}: fwAlertConfig_objectCreateSummary(), function declared void returns a value

  07/08/2012 Marco Boccioli
  - @jira{FWCORE-3097}: adapt to set of values for discrete alert.
  _fwAlertConfig_prepareAnalogDiscrete() detects if the values to match are a range (i.e.: 5-9) or a set (i.e.: 2,4,5). If set, converts values to array of int.

  11/05/2012 Marco Boccioli
  - @jira{FWCORE-3093}: improved documentation.
  - @jira{FWCORE-3094}: on fwAlertConfig_objectGetMany(), ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_VALUE] = 0 changed to ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_VALUE] = FALSE;
...
string
  @par Usage
Optional. For analog alarms. Must be a positive value. Default value is 0.0 and means no hysteresis.
See use example on fwAlertConfig_objectSet().
**/
...
string
  @par Usage
Optional. For analog alarms. Must be a negative value. Default value is 0.0 and means no hysteresis.
See use example on fwAlertConfig_objectSet().
**/
...
/**
  @ingroup fwConfigsAlert
  @par Description
Hysteresis time limit configuration item.
  @par Parameter Type
string
  @par Usage
Optional. For analog alarms. Default value is 0.0 and means no hysteresis.
See use example on fwAlertConfig_objectSet().
**/
//!Time hysteresis limit
const int fwAlertConfig_ALERT_LIMIT_HYSTERESIS_TIME = 14;
...
global mapping fwAlertConfig_configFileCache;
...
_fwAlertConfig_objectCreateParams(dyn_dyn_anytype &alarmParams, dyn_dyn_anytype &alarmLimits, int totalRanges)
{
	...
	{
		alarmLimits[i][fwAlertConfig_ALERT_LIMIT_TEXT] = (langString)"";
		...
		alarmLimits[i][fwAlertConfig_ALERT_LIMIT_TEXT_WENT] = (langString)"";
		alarmLimits[i][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_UP] = 0.0;
		alarmLimits[i][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_LOW] = 0.0;
		alarmLimits[i][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_TIME] = (time)0;
	}
	...
	alarmParams[fwAlertConfig_ALERT_PARAM_HELP][1] = (langString)"";
...
{
    if (dynlen(paramsObject)) dynClear(paramsObject); // FWCORE-3153
...
@param alertLimits					The limits of each range given here eg. makeDynFloat( 20, 60 ); Please note that the amount of limits is = amount of ranges-1. The object created will have an array containing the first limit as a fake limit. In this example, the values will be 0,20,60 (0 is a fake element just to fill the array).
								 ...
								 bool storeInParamHistory = TRUE)
{
	...
	ddmAlertParams[fwAlertConfig_ALERT_PARAM_HELP][1] = (langString)alertHelp;
	...
	for(i = 1 ; i <= iRanges ; i++)
	{
		ddmAlertLimits[i][fwAlertConfig_ALERT_LIMIT_TEXT] = (langString)alertTexts[i];
								  ...
								  dyn_string &exceptionInfo)
{
	...
	if(!fwAlertConfig_objectConfigExists(alertConfigObject, configType, exceptionInfo))
	{
		...
		alertHelp = "";
		limitsIncluded = makeDynBool(false);
		isActive = FALSE;
		return;
	}
...
// DebugN("configsToDeactivate:",configsToDeactivate);
	...
	for(i = 1; i <= length; i++)
	{
		...
		dynClear(dfHysteresisLimits);
		dynClear(hysteresisTimes);
...
//       dynAppend(dpesToActivate,dpes[i]);
		for(j = 1 ; j <= ranges ; j++)
		{
            ...
            // FWQA-413, need compatibility if function called with data structure filled WITHOUT time-hysteresis
            time tHyst=0;
            if (dynlen(ddmAlertLimit[j])>=fwAlertConfig_ALERT_LIMIT_HYSTERESIS_TIME) {
                tHyst=ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_TIME];
            }
			hysteresisTimes[j] = tHyst;
			if(j <= ranges - 1)
			{
				dfHysteresisLimits[((j - 1) * 2) + 1] = ddmAlertLimit[j + 1][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_UP];
				dfHysteresisLimits[((j - 1) * 2) + 2] = ddmAlertLimit[j + 1][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_LOW];
			}
		}
...
//          DebugN("setting analog discrete");
					_fwAlertConfig_prepareAnalogDiscrete(dpes[i], currentConfigTypes[i], numberOfRanges[i],
														 alertHandlingTypes, dsLimitMatch, dsAlertTexts, dsAlertClasses, hysteresisTimes,
...
//          DebugN("setting analog");
					...
					_fwAlertConfig_generateExtraAnalogConfig(ranges, dfAlertLimits, alertHandlingTypes,
							dfHysteresisLimits, hysteresisTimes, diHysteresisType, dbIncludeLimitsDummy, exceptionInfo);
//          DebugN("dfHysteresisLimits: "+dfHysteresisLimits);
					_fwAlertConfig_prepareAnalog(dpes[i], currentConfigTypes[i], numberOfRanges[i],
												 ...
												 diHysteresisType, dfHysteresisLimits, hysteresisTimes, tempAttributes, tempValues,
...
// DebugN("dsAlertClasses: "+dsAlertClasses);
				...
				if(bDiscreteAlert)
				{
					_fwAlertConfig_prepareDigitalDiscrete(dpes[i], currentConfigTypes[i],
														  dsAlertTexts, dsAlertClasses, hysteresisTimes,
														  minimumPriority, sAlertPanel, dsAlertPanelParameters, sAlertHelp,
				...
				{
				    // Note! non-discrete digital alarm has no WENT texts
					_fwAlertConfig_prepareDigital(dpes[i], currentConfigTypes[i], dsAlertTexts, dsAlertClasses, hysteresisTimes,
												  minimumPriority, sAlertPanel, dsAlertPanelParameters, sAlertHelp,
...
//       DebugN("dsSummaryDpeList: "+dsSummaryDpeList);
		...
		{
           //DebugTN(__FUNCTION__,"Setting...", bStoreInParamHistory, attributes, values);
...
fwAlertConfig_objectGet(string dpe, dyn_mixed &alertConfigObject, dyn_string &exceptionInfo)
{
	...
	dsDpe[1] = dpe;
    fwAlertConfig_objectInitialize(alertConfigObject, 1);
	fwAlertConfig_objectGetMany(dsDpe, ddmAlertConfigObjects, localExceptionInfo);
    if (dynlen(localExceptionInfo)) {
		fwException_raise(exceptionInfo, "ERROR", "fwAlertConfig_objectGet(): Could not get some of the attributes. Function aborted.", "");
...
The function caches the value once read from the config file.
The function is a functional replacement for the isAlertFileringActive()
which is delivered by aes.ctl (yet loading it pulls in lots of other depencencies).

*/
bool fwAlertConfig_isAlertFilteringActive()
{
  const string configEntry = "activateAlertFiltering";
  const string configFilePath = PROJ_PATH+"config/config";
  if (fwAlertConfig_configFileCache.contains(configFilePath))
  {
    anytype value = fwAlertConfig_configFileCache[configFilePath].value(configEntry, "");
  ...
  {
    fwAlertConfig_configFileCache[configFilePath] = makeMapping();
  }

  int afActive = 0;
  int rc = paCfgReadValue(configFilePath, "general", configEntry, afActive);
  bool filteringAcitve = rc < 0 || afActive > 0;
  fwAlertConfig_configFileCache[configFilePath][configEntry] = filteringAcitve;
  return filteringAcitve;
}
...
*/
fwAlertConfig_objectGetMany(const dyn_string &dpes, dyn_dyn_mixed &alertConfigObjects, dyn_string &exceptionInfo)
{
	...
	dyn_int diHysteresisType;
	dyn_float dfLowLimit, dfUpLimit;
	dyn_time hysteresisTimes;
    dyn_string localExceptionInfo;
	length = dynlen(dpes);

    dynClear(alertConfigObjects);

	_fwConfigs_getConfigTypeAttribute(dpes, fwConfigs_PVSS_ALERT_HDL, currentConfigTypes, localExceptionInfo);
    if (dynlen(localExceptionInfo)) {
		fwException_raise(exceptionInfo, "ERROR", "fwAlertConfig_objectGetMany(): Could not get some of the attributes. Function aborted.", "");
        dynAppend(exceptionInfo,localExceptionInfo);
		return;
	}
	...
	for(i = 1; i <= length; i++)
	{
		...
		switch(currentConfigTypes[i])
		{
			...
			case DPCONFIG_ALERT_BINARYSIGNAL:
				...
				k++;
				dsDpeAttr[k] = sDpe + ":_alert_hdl.._hyst_type";
				k++;
				dsDpeAttr[k] = sDpe + ":_alert_hdl.._hyst_time";
				k++;
				n++;
				break;
			case DPCONFIG_ALERT_NONBINARYSIGNAL:
				...
				for(j = 1; j <= diAlertRanges[i]; j++)
				{
					...
					else//if alarm is not discrete, get other attributes
					{
						...
						k++;
						dsDpeAttr[k] = sDpe + ":_alert_hdl." + j + "._l_limit";
						k++;
						dsDpeAttr[k] = sDpe + ":_alert_hdl." + j + "._u_incl";
						k++;
						dsDpeAttr[k] = sDpe + ":_alert_hdl." + j + "._hyst_type";
						k++;
						dsDpeAttr[k] = sDpe + ":_alert_hdl." + j + "._u_hyst_limit";
						k++;
						dsDpeAttr[k] = sDpe + ":_alert_hdl." + j + "._l_hyst_limit";
						k++;
					}
					dsDpeAttr[k] = sDpe + ":_alert_hdl." + j + "._hyst_time";
					k++;
				}
				dsDpeAttr[k] = sDpe + ":_alert_hdl.._active";
				k++;
...
//  DebugN("dmDpeAttrVal: ",dsDpeAttr,dmDpeAttrVal);
	...
	k = 1;
	n = 1; // FWCORE-3514: remember to increment it for everything except DPCONFIG_NONE!
	for(i = 1; i <= length; i++)
	{
...
//     DebugN("dmDpeType[i]: "+dmDpeType[i]);
		...
		switch(currentConfigTypes[i])
		{
			...
			case DPCONFIG_ALERT_BINARYSIGNAL:
				...
				k++;
				int hysteresisType = dmDpeAttrVal[k]; // :_alert_hdl.._hyst_time;
				k++;
				ddmAlertLimit[1][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_TIME] = (iDigitalOKRange == 0) ? 0 : dmDpeAttrVal[k]; // :_alert_hdl.._hyst_time;
				ddmAlertLimit[2][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_TIME] = (iDigitalOKRange == 0) ? dmDpeAttrVal[k] : 0; // :_alert_hdl.._hyst_time;
				k++;
				if (hysteresisType == DPATTR_HYST_NONE)
				{
					ddmAlertLimit[1][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_TIME] = 0;
					ddmAlertLimit[2][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_TIME] = 0;
				}
				n++; // FWCORE-3514; update the index used to look up the type in dmDpeType array!
				break;
			case DPCONFIG_ALERT_NONBINARYSIGNAL:
...
//         DebugBreak();
				...
				for(j = 1; j <= diAlertRanges[i]; j++)
				{
					...
					if(dmDpeType[n] == DPDETAIL_RANGETYPE_MATCH || dmDpeType[n] == DPDETAIL_RANGETYPE_SET) //if alarm is discrete, get some attributes
					{
						...
						k++;
						ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_TIME] = dmDpeAttrVal[k];//= ":_alert_hdl."+j+"._hyst_time";
						k++;
					}
					else
					{
						if(j == 1)
						{
							...
							ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_VALUE_INCLUDED] = FALSE; // first limit is null (#limits=#classes-1)
						}
						dfUpLimit[j]  = dmDpeAttrVal[k];// = sDpe+":_alert_hdl.<i>._u_limit";
						if(j < diAlertRanges[i])
						{
							ddmAlertLimit[j + 1][fwAlertConfig_ALERT_LIMIT_VALUE] =  dfUpLimit[j];
						}
						k++;
						dfLowLimit[j] = dmDpeAttrVal[k];// = sDpe+":_alert_hdl.<i>._l_limit";
						k++;
						if(j < diAlertRanges[i])
						{
						...
						k++;
						diHysteresisType[j] = dmDpeAttrVal[k];// = sDpe+":_alert_hdl."+j+"._hyst_type";
						k++;
						if(diHysteresisType[j] == DPATTR_HYST_VALUE || diHysteresisType[j] == DPATTR_HYST_TIME) //get hysteresis values
						{
							ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_UP]  = dmDpeAttrVal[k];//= ":_alert_hdl."+j+"._u_hyst_limit";
							k++;
							ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_LOW] = dmDpeAttrVal[k];//= ":_alert_hdl."+j+"._l_hyst_limit";
							k++;
							ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_TIME] = dmDpeAttrVal[k];//= ":_alert_hdl."+j+"._hyst_time";
							k++;
//             	DebugN(j, "_u_limit", dfUpLimit[j], "_l_limit", dfLowLimit[j],
//                         "_hyst_type", diHysteresisType[j],"_u_hyst_limit",ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_UP] ,
//                         "_l_hyst_limit",ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_LOW]);
						}
						else //hysteresis disabled
						{
							ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_UP]  = 0.0;//= ":_alert_hdl."+j+"._u_hyst_limit";
							k++;
							ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_LOW] = 0.0;//= ":_alert_hdl."+j+"._l_hyst_limit";
							k++;
							ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_TIME] = (time)0;//= ":_alert_hdl."+j+"._hyst_time";
							k++;
						}
					}
				}
				//if alarm is analog, compute relative hysteresis values
				if(dmDpeType[n] != DPDETAIL_RANGETYPE_MATCH && dmDpeType[n] != DPDETAIL_RANGETYPE_SET)
				{
					for(j = diAlertRanges[i]; j >= 2; j--)
					{
//             setDbgFlag("CTRL_DEBUGBREAK");
//             DebugBreak();
						// additional processing of hyst type: we need to look one range down as well...
						if(diHysteresisType[j - 1] != DPATTR_HYST_VALUE && diHysteresisType[j - 1] != DPATTR_HYST_TIME)
						{
							diHysteresisType[j] = diHysteresisType[j - 1];
						}

						if(diHysteresisType[j] == DPATTR_HYST_VALUE)
						{
							ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_UP] = ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_LOW] - dfLowLimit[j];
							ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_LOW]  = ddmAlertLimit[j - 1][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_UP] - dfUpLimit[j - 1];
						}
						else
						{
							ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_UP] = 0.0;
							ddmAlertLimit[j][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_LOW] = 0.0;
						}
					}
				}

				// reset the first limit to dummy...
				ddmAlertLimit[1][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_LOW] = 0.0; // first limit is null (#limits=#classes-1)
				ddmAlertLimit[1][fwAlertConfig_ALERT_LIMIT_HYSTERESIS_UP] = 0.0;  // first limit is null (#limits=#classes-1)
			...
			case DPCONFIG_SUM_ALERT:
				...
				ddmAlertLimit[2][fwAlertConfig_ALERT_LIMIT_TEXT] = dmDpeAttrVal[k];//= sDpe+":_alert_hdl.._text1";
				k++;
				ddmAlertParam[fwAlertConfig_ALERT_PARAM_SUM_THRESHOLD][1] = fwAlertConfig_isAlertFilteringActive() ? dmDpeAttrVal[k] : 0;//= sDpe+":_alert_hdl.._filter_threshold";
...
//      DebugN("ddmAlertLimit: "+ddmAlertLimit);
	...
	if (dynlen(err))
	{
		fwException_raise(exceptionInfo, "ERROR", "Error in fwAlertConfig_objectGetMany, " + (string)err, "");
	}
							  ...
							  bool fallBackToSet = FALSE)
{
	...
	if (dynlen(hysteresisTimes) != 2)
	{
		fwException_raise(exceptionInfo, "ERROR", __FUNCTION__ + ": The alert configuration required for \"" + dpe + "\" is not consistent, wrong number of hysteresis time parameters", "");
		return;
	}
	...
	attributes = makeDynString(			dpe + ":_alert_hdl.._ok_range",
										...
										dpe + ":_alert_hdl.._hyst_type",
										dpe + ":_alert_hdl.._hyst_time"
                                        );

	time hysteresisTime = hysteresisTimes[!okRange + 1];
	int hysteresisType = hysteresisTime > 0 ? DPATTR_HYST_TIME : DPATTR_HYST_NONE;
	values = makeDynMixed(okRange, alertTexts[2], alertTexts[1], alertClass, hysteresisType, hysteresisTime);
...
_fwAlertConfig_generateExtraAnalogConfig(int ranges, dyn_float limits, dyn_int &alertHandlingTypes, dyn_float &hysteresisLimits, dyn_time &hysteresisTimes,
		dyn_int &hysteresisTypes, dyn_bool &includeLimits, dyn_string &exceptionInfo)
{
	int i, iUp, iLow;
	float fHysteresisUp, fHysteresisLow;
	dyn_float dfHysteresisValues = hysteresisLimits;
	dynClear(alertHandlingTypes);
	dynClear(hysteresisTypes);
	dynClear(includeLimits);

	// hysteresis type is same for all ranges: any time non zero -> time hysteresis, any hysteresis value non zero -> value hysteresis
	int nonZeroHystTimeCount = hysteresisTimes.count() - hysteresisTimes.count((time)0);
	int nonZeroHystValueCount = hysteresisLimits.count() - hysteresisLimits.count((float)0);
	int hysteresisType = nonZeroHystTimeCount > 0 ? DPATTR_HYST_TIME : (nonZeroHystValueCount > 0 ? DPATTR_HYST_VALUE : DPATTR_HYST_NONE);

	for(i = 1; i <= ranges; i++)
	{
		alertHandlingTypes[i] = DPDETAIL_RANGETYPE_MINMAX;
		hysteresisTypes[i] = hysteresisType;
	}

	//compute hysteresis values
	if(ranges >= 2)
	{
		i = 1;
		iUp = ((i - 1) * 2) + 1;
		iLow = ((i - 1) * 2) + 2;
		hysteresisLimits[1] = limits[1] + dfHysteresisValues[2];

		hysteresisLimits[2] = limits[1] + dfHysteresisValues[1];
	}
	if(ranges >= 3)
	{
		i = 2;
		iUp = ((i - 1) * 2) + 1;
		iLow = ((i - 1) * 2) + 2;
		hysteresisLimits[4] = limits[2] + dfHysteresisValues[3];

		hysteresisLimits[3] = limits[2] + dfHysteresisValues[4];
	}
	if(ranges >= 4)
	{
		i = 3;
		iUp = ((i - 1) * 2) + 1;
		iLow = ((i - 1) * 2) + 2;
		hysteresisLimits[6] = limits[3] + dfHysteresisValues[5];

		hysteresisLimits[5] = limits[3] + dfHysteresisValues[6];
	}
	if(ranges >= 5)
	{
		i = 4;
		iUp = ((i - 1) * 2) + 1;
		iLow = ((i - 1) * 2) + 2;
		hysteresisLimits[8] = limits[4] + dfHysteresisValues[7];

		hysteresisLimits[7] = limits[4] + dfHysteresisValues[8];
	}

//  DebugN("_fwAlertConfig_generateExtraAnalogConfig: hysteresisLimits",hysteresisLimits,"limits",limits);
...
@param hysteresisTimes	Alert hysteresis times for each range
@param attributes				Output - the list of attributes that need to be set is returned here
@param values						Output - the list of values that need to be set is returned here
@param exceptionInfo		Details of any exceptions are returned here
@param modifyOnly				Optional parameter - Default value FALSE.  Set to TRUE to modify alert without setting "type" attributes.
							  ...
							  dyn_string alertWentTexts = makeDynString())
{
	...
	const string hysteresisTimeAttribute = ":_alert_hdl.<i>._hyst_time";
...
// DebugN("limits: "+limits);
	...
	// PARA checks for the higher ranges delay times not to be smaller than the previous ones, "higher range" is further from the good range
	int goodRangeIndex = dynContains(alertClasses, "");
	for (i = goodRangeIndex + 1; i <= dynlen(hysteresisTimes); ++i)
	{
		int rangeToCompare = i - 1;
		if (hysteresisTimes[i] < hysteresisTimes[rangeToCompare])
		{
			// index is above the good one so the range index must be adapted for the error message
			string badRangeToReport = i - 1;
			string referenceRangeToReport = rangeToCompare - 1;
			string message = __FUNCTION__ + ": The alert hysteresis time for \"" + dpe + "\" are not consistent." +
												" The delay " + (long)hysteresisTimes[i] + " at range " + badRangeToReport + " (limit value " + limits[badRangeToReport] + ")" +
												" cannot be shorter than " + (long)hysteresisTimes[rangeToCompare] + " for range " + referenceRangeToReport + " (limit value " + limits[referenceRangeToReport] + ")";
			fwException_raise(exceptionInfo, "ERROR", message, "");
			return;
		}
	}

	for (i = goodRangeIndex - 1; i > 1; --i)
	{
    int rangeToCompare = i - 1;
		if (hysteresisTimes[i] > hysteresisTimes[rangeToCompare])
		{
			string message = __FUNCTION__ + ": The alert hysteresis time for \"" + dpe + "\" are not consistent." +
												" The delay " + (long)hysteresisTimes[rangeToCompare] + " at range " + rangeToCompare + " (limit value " + limits[rangeToCompare] + ")" +
												" cannot be shorter than " + (long)hysteresisTimes[i] + " for range " + i + " (limit value " + limits[i] + ")";
			fwException_raise(exceptionInfo, "ERROR", message, "");
			return;
		}
	}
	...
	if(!modifyExistingConfig)
	{
				...
				dynlen(hysteresisTimes) != ranges)
		{
			...
			DebugTN("_fwAlertConfig_prepareAnalog: dynlen(hysteresisType),	dynlen(alertHandlingType),	dynlen(includeLimits),dynlen(hysteresisLimits)",
					dynlen(hysteresisType),
					dynlen(alertHandlingType),
					dynlen(includeLimits),
					dynlen(hysteresisLimits));
			return;
		}
	...
	else
	{
		if(noWentText)//avoid writing the attribute _went_text (not existing on PVSS 3.6)
		{
			...
			attributeTemplates = makeDynString(alertHandlingTypeAttribute, hysteresisTypeAttribute,
											   ...
											   hysteresisLowerLimitAttribute, hysteresisUpperLimitAttribute, hysteresisTimeAttribute);
			//build array of most of the values to be written
			desiredValues = makeDynMixed(configType,
										 ...
										 lowerHystLimits, upperHystLimits, hysteresisTimes);
		}
		else
		{
			//build list of attribute templates from globals constants
			attributeTemplates = makeDynString(alertHandlingTypeAttribute, hysteresisTypeAttribute,
											   ...
											   alertWentTextAttribute, hysteresisTimeAttribute);
			...
			desiredValues = makeDynMixed(configType,
										 ...
										 alertTexts, alertClasses,
										 TRUE, notIncludeLimits, includeLimits, TRUE,
										 lowerHystLimits, upperHystLimits, alertWentTexts, hysteresisTimes);
		}
...
// 		dynAppend(values, tempValue);
		dynAppend(values, desiredValues[i]);
	}
...
@param fallBackToSet			Optional parameter - Default value FALSE.  Used in combination with modifyOnly.
						...
						If modifyOnly = TRUE and fallBackToSet = TRUE, then if the alert config is not compatible, modifyOnly is treated as if it was FALSE.
@param alertWentTexts				Optional parameter - the list of went texts, one per range.
// TODO alarm delay parameter -> no time (0) -> no hysteresis, time > 0 -> hyst type = 2
*/
...
//   DebugN(ranges);
	...
	for(i = 1; i <= length; i++)
	{
		...
		db64stateBits[i] = (bit64) stateBits[i];
	}
...
//    DebugN(stateBits);
	if(modifyExistingConfig)
	{
		...
		attributeTemplates = makeDynString(alertHandlingTypeAttribute,
										   ...
										   alertTextAttribute,
										   alertClassAttribute,
										   hysteresisTimeAttribute);

		//build array of most of the values to be written
		desiredValues = makeDynMixed(alertHandlingType, dsValuesMatch, alertTexts, alertClasses, hysteresisTimes);
	}
	else
	{
		//build list of attribute templates from globals constants
		...
		desiredValues = makeDynMixed(configType, alertHandlingType,
									 ...
									 discreteNegation, db64stateBits, stateMatch, alertWentTexts, hysteresisTimes);
	...
	//************WARNING: this piece of code was temporarily disabled because of the bug on PVSS 3.8.2, as described on ETM-824.
	//See also FWCORE-3097 for its resolution.
	dyn_int di_setValues;
	for(i = 1; i <= dynlen(values); i++)
	{
		if(patternMatch("*._match", attributes[i]) && patternMatch("*,*", values[i]))
		{
			...
			//the values for _set must be casted to int
			for(j = 1 ; j <= dynlen(values[i]) ; j++)
			{
				di_setValues[j] = values[i][j];
			}
			values[i] = di_setValues;
			values[i - ranges] = DPDETAIL_RANGETYPE_SET; //set attribute _type
		}
	}
//   DebugN(attributes, values);
//   ***********This line is put TEMPORARILY instead, in order to warn about the limitation:
//   for(i=1; i<=dynlen(values); i++)
// 	{
//     if(patternMatch("*._match",attributes[i]) && patternMatch("*,*", values[i]))
//     {
//       fwException_raise(exceptionInfo, "ERROR", "The coma-separated set of values is not currently supported. Dpe: "+attributes[i] + " ; values: "+values[i],"");
//     }
//   }
                                      ...
                                      dyn_string alertWentTexts = makeDynString())
{
    ...
    if (dynlen(alertWentTexts)<2) alertWentTexts[2]=""; // if empty, initialize with empty strings
	...
	for(i = 1 ; i <= 2 ; i++)
	{
		...
		dynAppend(values, hysteresisTimes[i] > 0 ? DPATTR_HYST_TIME : DPATTR_HYST_NONE);
		dynAppend(attributes, dpe + ":_alert_hdl." + i + "._hyst_type");
		dynAppend(values, hysteresisTimes[i]);
		dynAppend(attributes, dpe + ":_alert_hdl." + i + "._hyst_time");
        dynAppend(values, alertWentTexts[i]);
		dynAppend(attributes, dpe + ":_alert_hdl." + i + "._went_text");
				   ...
				   dyn_string &alertPanelParameters, string &alertHelp, bool &alertActive, dyn_string &exceptionInfo)
{
	...
	if(alertType == DPCONFIG_ALERT_BINARYSIGNAL)
	{
		...
		if(digitalOKRange == 0)
		{
			alertClass = makeDynString("", aClass);
		}
		else
		{
			alertClass = makeDynString(aClass, "");
	...
	else if(alertType == DPCONFIG_SUM_ALERT)
	{
		...
		dpGet(dpe + ":_alert_hdl.._dp_list", dpeList,
			  ...
			  dpe + ":_alert_hdl.._panel_param", alertPanelParameters,
			  dpe + ":_alert_hdl.._active", alertActive,
			  dpe + ":_alert_hdl.._filter_threshold", filterThreshold);
							  ...
							  bool fallBackToSet = FALSE)
{
	...
	if(!modifyExistingConfig)
	{
		...
		// FWCORE--3370 add _dp_pattern but only if it is not there yet!
		if (!dynContains(attributes,dpe + ":_alert_hdl.._dp_pattern")) {
		    dynAppend(extraAttributes,dpe + ":_alert_hdl.._dp_pattern");
		    dynAppend(extraValues, "");
		}
...
_fwAlertConfig_checkClassPriorities(dyn_string systemsToCheck, dyn_string &alertClasses, bool &classesOk, dyn_string &exceptionInfo, bool useAlternativeClassPriorities = FALSE)
{
	...
	{
		int pos = dynContains(systemsToCheck, "");
