...
@par Modification History
  ...
  - @sav{85462}: Functions *Many() with parameters as reference. This improves the performances (less memory consumption).
  - @sav{85880}: Improved performance for fwDpFunction_objectGetMany(). New function written, with identical signature. The old function is commented out.

  23/06/2011 Marco Boccioli
  - @jira{ENS-3098}: Improve performance in setting dpFunctions.
  - @jira{ENS-3174}: Support for statistical dpFunctions.
    Added new functions:
...
*/
void _fwDpFunction_checkFunction(const string &function, const dyn_string &parameters, const dyn_string &globals, bool &isOk, dyn_string &exceptionInfo)
{
  const string localParameterPattern = "(p([1-9][0-9]*))";
  const string localParameterErrorPrefix = "_fwDpFunction_checkFunction(): Paramter p";
  isOk = _fwDpFunction_checkFunctionCodeParameterReferences(localParameterPattern, function, dynlen(parameters), localParameterErrorPrefix, exceptionInfo);
  if (!isOk)
  {
    return;
  }

  const string globalParameterPattern = "(g([1-9][0-9]*))";
  const string globalParameterErrorPrefix = "_fwDpFunction_checkFunction(): Global g";
  isOk = _fwDpFunction_checkFunctionCodeParameterReferences(globalParameterPattern, function, dynlen(globals), localParameterErrorPrefix, exceptionInfo);
}

/**
 * Look for the given number pattern in the code and check that it's within the specified range
 *
 * @param pattern regular expression to capture numbered parameter in the code, e.g. (p([1-9][0-9]*))
 *                to find parameter like p1, ..., p9, p10, ...
 * @param code DP function code to check
 * @param parametersCount number of defined parameters in the DP function
 * @param errorPrefix prefix to use in case an error is observed
 * @param exceptionInfo	details of any errors
 * @return true if all occurences of pattern belong to the range defined by the parametersCount, false otherwise
 */
private bool _fwDpFunction_checkFunctionCodeParameterReferences(const string& pattern,
                                                                const string& code,
                                                                int parametersCount,
                                                                ...
                                                                dyn_string& exceptionInfo)
{
  ...
  {
    mapping options = makeMapping("startPosition", lastParameterPosition + 1);

    dyn_string result;
    lastParameterPosition = regexpSplit(pattern, code, result, options);
    parameterFound = lastParameterPosition > -1;
    if (parameterFound && dynlen(result) == 3 && ((int) result[3] == 0 || (int) result[3] > parametersCount))
    {
      fwException_raise(exceptionInfo, "ERROR", errorPrefix + (int) result[3] + " is not defined in the parameter list.", "");
      return FALSE;
    }
  }
...
@reviewed 2018-08-14 @whitelisted{API}
*/
							...
							bool runChecks = TRUE)
{
	...
	dpes[1] = dpe;
	fwDpFunction_objectSetMany(dpes, fos, exceptionInfo, runChecks);
}
...
//old version of fwDpFunction_objectGetMany.
...
@param dpes				List of dpes from which to extract the dpFunction. Passed as reference only for performance reasons. Not modified.
@param configExists         True if dp config exists, false if dp config does not exist
							...
							dyn_string &exceptionInfo)
{
	...
	int length;
	dyn_mixed functionObject;
	dyn_string dsTypesAttr, dsDpAttr;
	dyn_mixed dsTypesVal, dsTempVal, dsAttrVal;
	...
	for(i = 1 ; i <= length ; i++)
	{
		dynAppend(dsTypesAttr, dpes[i]  + ":_dp_fct.._type");
	...
	//get the parameters of all the configured dpes, according to the function pocedure:
	for(i = 1 ; i <= length ; i++)
	{
		switch(dsTypesVal[i])
		{
			case DPCONFIG_STAT_FUNCTION:
				dynAppend(dsDpAttr, dpes[i] + ":_dp_fct.._param");
				dynAppend(dsDpAttr, dpes[i] + ":_dp_fct.._global");
				dynAppend(dsDpAttr, dpes[i] + ":_dp_fct.._fct");
				dynAppend(dsDpAttr, dpes[i] + ":_dp_fct.._stat_type");
				dynAppend(dsDpAttr, dpes[i] + ":_dp_fct.._interval");
				dynAppend(dsDpAttr, dpes[i] + ":_dp_fct.._delay");
				dynAppend(dsDpAttr, dpes[i] + ":_dp_fct.._read_archive");
				break;
			...
			default:
				fwException_raise(exceptionInfo, "ERROR", "fwDpFunction_objectGetMany(): DP function type (" + dsTypesVal[i] + ") on dpe '" + dpes[i] + "' not suppported", "");
				break;
		}
...
//       DebugN("dsDpAttr:",dsDpAttr);
	...
	//set the parameters of all the configured dpes to the param object
	j = 1;
	for(i = 1 ; i <= length ; i++)
	{
		fwDpFunction_objectInitialize(functionObject);
		switch(dsTypesVal[i])
		{
			case DPCONFIG_STAT_FUNCTION:
				configExists[i] = TRUE;
				functionObject[fwDpFunction_OBJ_TYPE] = dsTypesVal[i];//_type
				functionObject[fwDpFunction_OBJ_PARAM] = dsAttrVal[j];
				j++;//_param
				functionObject[fwDpFunction_OBJ_GLOBAL] = dsAttrVal[j];
				j++;//_global
				functionObject[fwDpFunction_OBJ_FUNCTION] = dsAttrVal[j];
				j++;//_fct
				functionObject[fwDpFunction_OBJ_STAT_TYPE] = dsAttrVal[j];
				j++;//_stat_type
				functionObject[fwDpFunction_OBJ_STAT_INTERVAL] = dsAttrVal[j];
				j++;//_interval
				functionObject[fwDpFunction_OBJ_STAT_DELAY] = dsAttrVal[j];
				j++;//_delay
				functionObject[fwDpFunction_OBJ_STAT_READ_ARCHIVE] = dsAttrVal[j];
				j++;//_read_archive
				break;
			...
			default:
				configExists[i] = FALSE;	//unsupported dp function type shown as not existing
				break;
		}
		//add the configuration object in the array of objects
		functionObjects[i] = functionObject;
	}
}
//@}
