 ...
 * @ingroup Constants
*/
const string csFwInstallationToolVersion = "9.3.1";
 ...
 * @ingroup Constants
*/
const string csFwInstallationLibVersion = "9.3.1";
...
int fwInstallation_verifyDestinationDirAndSourceFiles(string componentName,
  ...
  if(!registerOnly){
    string strError;
    int retVal = fwInstallation_verifyDirectory(destinationDir);
    if(retVal < 0){
      strError = "Component installation folder: '" + destinationDir + "' does not exist";
    }else if(access(destinationDir, W_OK) != 0){
      strError = "Component installation folder: '" + destinationDir + "' is not write enabled";
    }
    if(!strError.isEmpty()){
      dynAppend(strErrors, strError);
      fwInstallation_throw(strError);
    }
  }
...
/** This function stores in the component internal dp of the installation tool the list of post install scripts to be run with names
 ...
 @param component (in) Name of component with postinstall scripts
 @param componentPostInstalls (in) list of post-install files to be stored
 @param componentInstallationDir (in) directory in which component is installed
 @return 0 if OK, -1 if error
*/
int fwInstallation_storeComponentPostInstallScripts(string component, const dyn_string &componentPostInstalls,
                                                    string componentInstallationDir)
{
  string dp = fwInstallation_getInstallationPendingActionsDp();

  dyn_string allRegisteredPostInstallScripts;
  int retVal = dpGet(dp + ".postInstallFiles", allRegisteredPostInstallScripts);
  if(retVal != 0 || !getLastError().isEmpty()){
    return -1;
  }

  for(int i = 1; i <= dynlen(componentPostInstalls); i++) {
    string postInstallScript = componentPostInstalls[i];
    if(postInstallScript.startsWith("./")){
      postInstallScript = postInstallScript.mid(2);
    }
    string postInstallScriptAbsPath = componentInstallationDir + "/" + postInstallScript;
    string postInstallEntry = component + "|" + postInstallScriptAbsPath;
    if(!allRegisteredPostInstallScripts.contains(postInstallEntry)){
      allRegisteredPostInstallScripts.append(postInstallEntry);
    }
  }

  retVal = dpSetWait(dp + ".postInstallFiles", allRegisteredPostInstallScripts);
  if(retVal != 0 || !getLastError().isEmpty()){
    return -1;
  }
  return 0;
...
const int FW_INSTALLATION_POSTINSTALL_ACTION_BIT_QT_HELP_PREPARE = 5; // used only for status reporting
                                    ...
                                    bool installSubComponents = true)
{
  ...
  // get the directory where the component will be installed
  string destinationDir = fwInstallation_getInstallationDirectory();
  ...
  dyn_string dynPostDeleteFiles         = parsedComponentInfo[FW_INSTALLATION_XML_COMPONENT_POST_DELETE_SCRIPTS_CURRENT];  // note: here CURRENT suffix is needed for a relative path (otherwise an absolute path is returned)
  dyn_string dynPostInstallFiles        = parsedComponentInfo[FW_INSTALLATION_XML_COMPONENT_POST_INSTALL_SCRIPTS_CURRENT]; // same note as above
...
// add scripts to the fwScripts.lst file
  ...
  if(error == 0){
    ...
    if(fwInstallation_storeComponentPostInstallScripts(componentName, dynPostInstallFiles, destinationDir)){
...
int fwInstallation_createPath(string sPath, bool createDirectory = true)
{
  ...
  if(access(sPath, F_OK) == -1 && createDirectory){
    ...
    if(access(sPath, F_OK) != -1){
      ...
      fwInstallation_throw("Failed to create directory: " + sPath + ". Path will not be " +
                           "added to the project config file");
      return -1;
    }
  }
  // Directory has been created - add it into the config file
  if(fwInstallation_addProjPath(sPath, 999)){
    fwInstallation_throw("Failed to add path: " + sPath + " to the pvss_path entry in the " +
                         "project config file");
    return -1;
  }
  // Set it in the tool configuration DP as the component installation directory
  return fwInstallation_setInstallationDirectory(sPath);
}


/** Get installation directory path from the tool configuration datapoint
  *
  * @return Installation directory path
  */
string fwInstallation_getInstallationDirectory()
{
  string installationDirPath;
  dpGet(fwInstallation_getInstallationDp() + ".installationDirectoryPath", installationDirPath);
  dyn_errClass err = getLastError();
  if(!err.isEmpty()){
    fwInstallation_throw(getErrorText(err));
  }
  return installationDirPath;
}

/** Set installation directory path in the tool configuration datapoint
  *
  * @param installationDirPath (in)  Installation directory path
  * @return 0 if success, -1 in case of an error
  */
int fwInstallation_setInstallationDirectory(string installationDirPath)
{
  int retVal = dpSetWait(fwInstallation_getInstallationDp() + ".installationDirectoryPath",
                         installationDirPath);
  dyn_errClass err = getLastError();
  if(!err.isEmpty()){
    fwInstallation_throw(getErrorText(err));
...
/** This function verifies the given directory path. Checks if it exists and if it is a directory
 @param directoryPath (in) Directory path to check
 @return  0 if given path is an existing directory (verification OK),
         -1 if empty string given
         -2 if path does not exist
         -3 if path is a file and not a directory
*/
int fwInstallation_verifyDirectory(string directoryPath){
    ...
    if(!isdir(directoryPath)){
        return -3; // path is not a directory
    }
    return 0;
}

/** This function check the installation directory for all installed components and returns
  * internal installation DP of components for which the installation directory does not exist
  * in the filesystem
  * @param onlyCurrentReduPeer (in) Effective only for redundant systems - flag indicating whether
  *                                 only components installed on the current redu peer should be returned.
  *                                 If false (default) component DPs from both peers are returned.
  * @return List of internal fwInstallation DPs for installed components having wrong directory
  */
dyn_string fwInstallation_getComponentsWithWrongInstallationDir(bool onlyCurrentReduPeer = false){
  dyn_string componentDps;
  if(onlyCurrentReduPeer){
    componentDps = fwInstallation_getInstalledComponentDps();
  }else{
    componentDps = fwInstallation_getInstalledComponentDps(1);
    dynAppendConst(componentDps, fwInstallation_getInstalledComponentDps(2));
  }
  dyn_string componentInstallationDirDpes;
  if(componentDps.isEmpty()){ // no components installed
    return makeDynString();
  }
  for(int i=0;i<componentDps.count();i++){
    componentInstallationDirDpes.append(componentDps.at(i) + ".installationDirectory");
  }
  dyn_string componentInstallationDirs;
  dpGet(componentInstallationDirDpes, componentInstallationDirs);
  dyn_string componentDpsToReturn;
  for(int i=0;i<componentInstallationDirs.count();i++){
    if(fwInstallation_verifyDirectory(componentInstallationDirs.at(i)) != 0){
      componentDpsToReturn.append(componentDps.at(i));
    }
  }
  return componentDpsToReturn;
}

/** For a given installed component DP it finds a path, from the list of provided directories,
  * in which component files are present. Note: it searches from last path given and returns
  * immediately when a checked path contains component files (doesn't check following paths)
  * @param componentDp (in) Internal fwInstallation DP representing installed component
  * @param dirPaths (in) List of paths to check for the presence of component files; if empty
  *                      (default) then all proj_path in config file (except PROJ_PATH) are checked
  * @return Path to directory containing component files or empty string in case of errors or when
  *         component files are not found in any of the checked directories
*/
string fwInstallation_findInstalledComponentXmlInPaths(string componentDp,
                                                       dyn_string dirPaths = makeDynString()){
  dyn_string projPaths;
  fwInstallation_getProjPaths(projPaths);
  if(projPaths.count() < 2){
    fwInstallation_throw("Failed to set update component installation directories - no " +
                         "installation directory found in project config file");
    return "";
  }
  projPaths.takeLast(); // remove PROJ_PATH from projPaths list

  string componentVersion, componentXml;
  int retVal = dpGet(componentDp + ".componentVersionString", componentVersion,
                     componentDp + ".descFile", componentXml);
  if(retVal != 0 || !getLastError().isEmpty()){
    fwInstallation_throw(__FUNCTION__ + ": Failed to read version and XML file name from " +
                         "component DP: " + componentDp, "WARNING");
    return "";
  }

  for(int i=dirPaths.count()-1;i>=0;i--){
    string projPath = dirPaths.at(i);
    string componentXmlPath = projPath + "/" + componentXml;
    if(access(componentXmlPath, F_OK) != 0){
      continue;
    }
    dyn_string dsValues;
    dyn_anytype daAttribs;
    fwInstallationXml_getTag(componentXmlPath, "version", dsValues, daAttribs);
    if(!dsValues.isEmpty() && dsValues.first() == componentVersion){
      return projPath;
    }
  }
  return "";
...
/** This functions sets in the given component DPs, the corresponding installation directory
  * @param componentDps (in) List of internal fwInstallation DPs of components to update
  * @param installationDirs (in) List of installation directories corresponding to componentDps list
  * @return 0 when success (Installation directories for given components were updated in internal DPs)
  *         -1 in case of error (lists size does not match, dpSetWait error)
*/
int fwInstallation_setComponentsInstallationDirs(dyn_string componentDps, dyn_string installationDirs){
  if(componentDps.count() != installationDirs.count()){
    fwInstallation_throw("Cannot set component installation dirs, list of component DPs does not " +
                         "match size of the list of installation directories");
    return -1;
  }
  dyn_string componentInstallationDirDpes;
  for(int i=0;i<componentDps.count();i++){
  ...
  int retVal = dpSetWait(componentInstallationDirDpes, installationDirs);
  if(retVal != 0 || !getLastError().isEmpty()){
    fwInstallation_throw("Failed to set component installation dirs - dpSetWait with error");
    return -1;
  }
  return 0;
}
...
/** Detects components not present in the registered installation directory and, if component files
  * are available in other proj_path, updates the installation directory to point to the new location
  * Example use case: Updating installation directory after moving the project to another host
  * @param requireUserInput (in) Flag indicating whether user confirmation is needed to complete
  *                              the action; default - false = don't require user confirmation
  * @param onlyCurrentReduPeer (in) Effective only for redundant systems - flag indicating whether
  *                                 only components installed on the current redu peer should be
  *                                 updated. If false (default) components from both peers are updated.
  * @return 0 when success (installation directories for all components found in new location were
  *         updated or there were no such components to update), 1 if user cancelled the action,
  *         -1 in case of errors (failed to set updated directories, no directories to search found
  *         in config file proj_path entries)
*/
int fwInstallation_updateComponentsInstallationDirs(bool requireUserInput = false,
                                                    bool onlyCurrentReduPeer = false){
  dyn_string projPaths;
  fwInstallation_getProjPaths(projPaths);
  if(projPaths.count() < 2){
    fwInstallation_throw("Failed to set update component installation directories - no " +
  ...
  dyn_string componentDpsToUpdate;
  dyn_string newInstallationDirs;
  dyn_string componentDpsNeedingUpdate =
          fwInstallation_getComponentsWithWrongInstallationDir(onlyCurrentReduPeer);
  for(int i=0;i<componentDpsNeedingUpdate.count();i++){
    string componentDp = componentDpsNeedingUpdate.at(i);
    string newInstallationDir =
            fwInstallation_findInstalledComponentXmlInPaths(componentDp, projPaths);
    if(!newInstallationDir.isEmpty()){
      componentDpsToUpdate.append(componentDp);
      newInstallationDirs.append(newInstallationDir);
    }
  }

  int retVal;
  if(!componentDpsToUpdate.isEmpty()){
    if(requireUserInput && myManType() == UI_MAN){
      string text = "Found " + componentDpsToUpdate.count() + " installed components which are " +
                    "located in the project path(s) different than at the moment of their " +
                    "installation. Would you like to update the registered installation " +
                    "directory for these components?";
      dyn_string ds;
      dyn_float df;
      ChildPanelOnCentralReturn("fwInstallation/fwInstallation_messageInfo.pnl",
                                "Update installation directory for installed components",
                                makeDynString("$text:" + text), df, ds);
      if(ds.isEmpty() || ds.first() != "OK"){
        return 1;
      }
    }
    retVal = fwInstallation_setComponentsInstallationDirs(componentDpsToUpdate, newInstallationDirs);
    if(retVal == 0){
      fwInstallation_throw("Updated registered installation directory for " +
                           componentDpsToUpdate.count() + " components", "INFO");
    }
  }

  int remainingComponentsNum = componentDpsNeedingUpdate.count() - componentDpsToUpdate.count();
  if(retVal == 0 && remainingComponentsNum > 0){
    fwInstallation_throw("There are " + remainingComponentsNum + " components for which " +
                         "installation directory cannot be updated as they are not found " +
                         "in project paths registered in config file. Run 'Check files' and " +
                         "consult the log to get these components", "WARNING");
  }
  return retVal;
}
                                   ...
                                   bool &deletionAborted)
{
  ...
  if(!deletionAborted){
    ...
    if(deleteAllFiles){
      for(int i=1;i<=dynlen(componentFiles);i++){
        ...
        if(hasPostDeleteFiles &&
           (postDeleteFiles.contains(componentFile) || // as of FWINS-2303 postDelete scripts are stored with the relative path
            fwInstallation_isPostDeleteScript(postDeleteFiles, installationDirectory, componentFile))){ // here handle postDelete registered with absolute path (installed prior to FWINS-2303)
          continue; // post-delete script must not be appended to the list of files that are removed - has to be kept until post-delete script is executed
        }
        if(access(installationDirectory + "/" + componentFile, F_OK) == 0){
          dynAppend(componentFilesDelete, componentFile);
        }else{
          fwInstallation_throw("Component " + componentName + " points to a non existing file: " +
                               installationDirectory + "/" + componentFile, "WARNING", 3);
        }
      }
    }
    ...
    if(hasPostDeleteFiles){
      if(componentFiles.contains(postDeleteFiles.first())){ // post delete files with a relative path
        fwInstallation_storePostDeleteScripts(postDeleteFiles, installationDirectory);
      }else{ // post delete files with an absolute path, don't prepend with installation dir path
...
/** Checks if component file, given with a relative path, is one of the post-delete scripts given in a list.
  * @param dynPostDeleteFiles (in)  List of absolute paths to post-delete files.
  ...
  */
bool fwInstallation_isPostDeleteScript(dyn_string dynPostDeleteFiles, string installationDirectory, string fileToCheck)
{
  fwInstallation_normalizePathList(dynPostDeleteFiles);
...
/** This function stores in the component internal dp of the installation tool the list of post delete scripts to be run
  * @param postDeleteFiles (in) list of post-delete files to be stored
  * @param installationDirectory (in)  Component installation directory
  * @return 0 if OK, -1 if error
  */
int fwInstallation_storePostDeleteScripts(const dyn_string &postDeleteFiles, string installationDirectory)
{
  string dp = fwInstallation_getInstallationPendingActionsDp();

  dyn_string allRegisteredPostDeleteScripts;
  int retVal = dpGet(dp + ".postDeleteFiles", allRegisteredPostDeleteScripts);
  if(retVal != 0 || !getLastError().isEmpty()){
    return -1;
  }

  for(int i = 1; i <= dynlen(postDeleteFiles); i++) {
    string postDeleteFile = postDeleteFiles[i];
    if(postDeleteFile.startsWith("./")){
      postDeleteFile = postDeleteFile.mid(2);
    }
    if(!installationDirectory.isEmpty()){
      postDeleteFile = installationDirectory + "/" + postDeleteFile;
    }

    if(access(postDeleteFile, R_OK) != 0){
      fwInstallation_throw("Post-delete script file: " + postDeleteFile + " is not accessible. " +
                           "Not appending to a post-delete routine", "WARNING");
      continue;
    }

    if(!allRegisteredPostDeleteScripts.contains(postDeleteFile)){
      allRegisteredPostDeleteScripts.append(postDeleteFile);
    }
  }

  retVal = dpSetWait(dp + ".postDeleteFiles", allRegisteredPostDeleteScripts);
  if(retVal != 0 || !getLastError().isEmpty()){
    return -1;
  }
  return 0;
