 ...
 * @ingroup Constants
*/
const string csFwInstallationDBLibVersion = "9.3.1";
...
//Edit: Functionality not supported by CtrlRdbAccess, constant may be removed in future releases
...
{
  dyn_dyn_mixed tmpData;
  int retVal = fwDbRunSQL(FW_INSTALLATION_DB_CONNECTION_NAME, sql, record, tmpData);
  dyn_errClass error = getLastError();
  if(!error.isEmpty()){
    fwInstallation_throw("fwInstallationDB_executeQuery() -> DB Error: " +
                         getErrorText(error) + " " + sql);
    return -1;
  }
  data = tmpData;
