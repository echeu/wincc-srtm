 ...
 * @ingroup Constants
*/
const string csFwInstallationDBUpgradeLibVersion = "9.3.1";
const string csFwInstallationDBUpgradeLibTag = "";
                                                       ...
                                                       string project = "")
{
  ...
  {
    fwInstallation_throw("ERROR: fwInstallationDBUpgrade_registerProjectToolUpgradeRequest() -> Could not check if host: " + host + " is registered in DB. Check DB connection parameters.");
    return -1;
  }
  else if(computer_id == -1)
  {
     fwInstallation_throw("ERROR: fwInstallationDBUpgrade_registerProjectToolUpgradeRequest() -> Host: " + host + " not yet registered in the DB.");
     return -1;
  }
  ...
  {
    fwInstallation_throw("ERROR: fwInstallationDBUpgrade_registerProjectToolUpgradeRequest() -> Could not check if project: " + project + " in host: " + host + " is registered in DB. Check DB connection parameters.");
    return -1;
  }
  else if(project_id == -1)
  {
     fwInstallation_throw("ERROR: fwInstallationDBUpgrade_registerProjectToolUpgradeRequest() -> Project " + project + " in host: " + host + " not yet registered in the DB.");
     return -1;
  }
...
//  record[5] = upgradeInfo[5];
  record[4] = project_id;
  record[5] = computer_id;

  //Check if project is registered:
  bool exists = false;
  if(fwInstallationDBUpgrade_isProjectToolUpgradeRequestRegistered(project_id, computer_id, exists) == -1)
  {
  ...
  if(exists)
  {
    sql = "UPDATE FW_SYS_STAT_TOOL_UPGRADE SET SOURCE_DIR = :1, TARGET_DIR = :2, SAFE_MODE = :3, REQUESTED_ON = SYSDATE " +
          "WHERE PROJECT_ID = :4 AND COMPUTER_ID = :5 AND EXECUTED_ON IS NULL";
  }
  else
  {
    sql = "INSERT INTO FW_SYS_STAT_TOOL_UPGRADE(ID, SOURCE_DIR, TARGET_DIR, SAFE_MODE, PROJECT_ID, REQUESTED_ON, COMPUTER_ID) " +
          "VALUES(FW_SYS_STAT_TOOL_UPGRADE_SQ.NEXTVAL, :1, :2, :3, :4, SYSDATE, :5)";
  }
...
int fwInstallationDBUpgrade_unregisterProjectToolUpgradeRequest(string host = "", string project = "")
{
  ...
  if(fwInstallationDB_isPCRegistered(computer_id, host) != 0)
  {
    fwInstallation_throw("fwInstallationDBUpgrade_unregisterProjectToolUpgradeRequest() -> Could not check if host: " + host + " is registered in DB. Check DB connection parameters.");
    return -1;
  }
  else if(computer_id == -1)
  {
     fwInstallation_throw("fwInstallationDBUpgrade_unregisterProjectToolUpgradeRequest() -> Host: " + host + " not yet registered in the DB.");
     return -1;
  }
  ...
  sql = "UPDATE FW_SYS_STAT_TOOL_UPGRADE SET EXECUTED_ON = SYSDATE WHERE PROJECT_ID = :1 AND COMPUTER_ID = :2 AND EXECUTED_ON IS NULL";
...
int fwInstallationDBUpgrade_isProjectToolUpgradeRequestRegistered(int project_id, int computer_id, bool &exists)
{
  ...
  sql = "SELECT project_id FROM fw_sys_stat_tool_upgrade WHERE project_id = :1 AND computer_id = :2 AND executed_on is null";
...
int fwInstallationDBUpgrade_getProjectToolUpgradeRequest(dyn_mixed &upgradeInfo, string host = "", string project = "")
{
  ...
  if(fwInstallationDB_isPCRegistered(computer_id, host) != 0)
  {
    fwInstallation_throw("ERROR: fwInstallationDBUpgrade_getProjectToolUpgradeRequest() -> Could not check if host: " + host + " is registered in DB. Check DB connection parameters.");
    return -1;
  }
  else if(computer_id == -1)
  {
     fwInstallation_throw("ERROR: fwInstallationDBUpgrade_getProjectToolUpgradeRequest() -> Host: " + host + " not yet registered in the DB.");
     return -1;
  }
  ...
  sql = "select source_dir, target_dir, safe_mode, requested_on, executed_on "
        + "from fw_sys_stat_tool_upgrade where project_id = :1 and computer_id = :2 and executed_on is null";
